...
import (
	...
	rootLog "zhanghefan123/security/logger"
...
func NewNetService(chainId string, localNet protocol.Net) *NetService {
...
// HandleMsgBusSubscriberOnMessage msgbus 的 onMessage 方法调用了这个方法来进行处理
...
func handleMsgBusSubscriberOnMessageBroadcast(
	...
	// 这里仅仅会进行共识消息的发送 NetMsg_CONSENSUS_MSG
	if (msgType == netPb.NetMsg_CONSENSUS_MSG) && !netService.isConsensusNodeIdListEmpty() {
...
func (ns *NetService) initBindMsgBus() error {
	...
	// 接受以 msgBusMsgFlagPrefix 前缀开头的消息, 并将其视为 NetMsg_CONSENSUS_MSG
	...
	// 接受以 msgBusConsensusTopicPrefix 前缀开头的消息，并将其视为 NetMsg_CONSENSUS_MSG
	if err := ns.receiveMsgForMsgBus(
		consensusMsgHandler,
		CreateFlagWithPrefixAndMsgType(
