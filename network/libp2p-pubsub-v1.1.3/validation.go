...
func (v *validation) Push(src peer.ID, msg *Message) bool {
	...
	if len(vals) > 0 || msg.Signature != nil {
		...
		default:
			v.p.log.Warnf("message validation throttled: queue full; dropping message from %s", src)
...
func (v *validation) validate(vals []*topicVal, src peer.ID, msg *Message) {
	...
	if msg.Signature != nil {
		if !v.validateSignature(msg) {
			v.p.log.Warnf("message signature validation failed; dropping message from %s", src)
...
loop:
	...
	if result == ValidationReject {
		v.p.log.Warnf("message validation failed; dropping message from %s", src)
	...
	if len(async) > 0 {
		...
		default:
			v.p.log.Warnf("message validation throttled; dropping message from %s", src)
...
func (v *validation) validateSignature(msg *Message) bool {
	...
	if err != nil {
		v.p.log.Debugf("signature verification error: %s", err.Error())
		return false
	}
...
func (v *validation) doValidateTopic(vals []*topicVal, src peer.ID, msg *Message, r ValidationResult) {
	...
	case ValidationAccept:
		v.p.sendMsg <- msg
	case ValidationReject:
		v.p.log.Debugf("message validation failed; dropping message from %s", src)
		v.tracer.RejectMessage(msg, rejectValidationFailed)
		return
	case ValidationIgnore:
		v.p.log.Debugf("message validation punted; ignoring message from %s", src)
		v.tracer.RejectMessage(msg, rejectValidationIgnored)
		return
	case validationThrottled:
		v.p.log.Warnf("message validation throttled; ignoring message from %s", src)
		v.tracer.RejectMessage(msg, rejectValidationThrottled)
...
func (v *validation) validateTopic(vals []*topicVal, src peer.ID, msg *Message) ValidationResult {
	...
	for _, val := range vals {
		...
		default:
			v.p.log.Debugf("validation throttled for topic %s", val.topic)
